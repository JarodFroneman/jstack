...
8. Implement only the human-selected direction and only when the governing workflow authorizes implementation. For JStack-led work, also follow its task mode and readiness path. Prefer semantic, platform-native controls and the project's proven components. Apply a JStack motion specification only when one governs the task.
...
- Read [liquid-glass.md](references/liquid-glass.md) only when the user requests a Liquid Glass direction or the existing product uses one. It is optional material guidance, not a default profile or a requirement for unrelated UI.
...
- In an explicitly GStack-led task, use this skill as design guidance only. GStack retains implementation, QA, review, and deployment ownership. Do not impose or claim JStack contracts, motion specifications, receipts, team or loop orchestration, or release gates unless the user separately chooses a JStack-led workflow. This boundary also applies to the selectively loaded references below: use their design and verification guidance without activating JStack protocol steps. Follow GStack's own evidence and approval requirements.
